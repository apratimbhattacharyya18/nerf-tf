...
def load_kitti(data_root,mode,dropout,block,coord_scale_factor,so3_representation,
    ...
    for index in tqdm(range(frame_range[1] - frame_range[0] + 1)):
        ...
        if img_num_curr in train_frames:
            ...
            R, t, fx, fy = cam_params_left.forward(index)
            tmat = np.eye(4)
    ...
    for index in tqdm(range(frame_range[1] - frame_range[0] + 1)):
        ...
        if img_num_curr in train_frames:
            ...
            R, t, fx, fy = cam_params_right.forward(index)
            tmat = np.eye(4)
            tmat[:3,:3] = R#.detach().cpu().numpy()
            tmat[:3,3] = t#.detach().cpu().numpy()
            cams.append(tmat.astype(np.float32))
    ...
    for video_im_num in range(video_len):
        R, t, fx, fy = cam_params_video.forward(video_im_num)
        tmat = np.eye(4)
    ...
    if lr_check:
        #multi_view_consistency( images, rays, poses, depths, H, W, K )
    ...
    if reg_pose_list is not None:
        ...
        print('reg_poses ',reg_poses.shape)
        #sys.exit(0)
    ...
    for i in range(H):
        for j in range(W):
            ...
            if (0 <= proj_i < H) and (0 <= proj_j < W):
                ...
                if np.abs(true_depth - proj_d) < 0.0025:
                    valid_mask[i,j] = True
