...
scene_bbox = tf.constant([[-40.5304, -34.3491, -8.15422], [ 111.638, 38.5536, 30.0 ]])
box_center = ((scene_bbox[0] + scene_bbox[1]) / 2)
box_size = tf.math.abs(scene_bbox[1] - scene_bbox[0])
...
def renormalize_to_unit_box(rays_pts):
    rays_pts = (rays_pts - box_center[None,:]) / (box_size[None,:] / 2)
    return rays_pts


def filtering( target_shape, outputs_flat, pts_mask ):
    #print('pts_mask ',pts_mask.shape)

    idx_remove = tf.where( tf.logical_not(pts_mask) )[:,-1]
    idx_keep = tf.where( pts_mask )[:,-1]
    
    values_remove = tf.zeros( [tf.shape(idx_remove)[0], outputs_flat.shape[1]] ) 
    values_keep = outputs_flat#tf.gather( outputs_flat, idx_keep )
    ...
    #print('idx_remove, idx_keep ',idx_remove.shape, idx_keep.shape)
    idx_remove = tf.concat([tf.stack( [ idx_remove, zeros_remove, ], axis=1 ),
                           tf.stack( [ idx_remove, zeros_remove + 1, ], axis=1 ),
                           tf.stack( [ idx_remove, zeros_remove + 2, ], axis=1 ),
                           tf.stack( [ idx_remove, zeros_remove + 3, ], axis=1 )], axis=0)
    idx_keep = tf.concat([tf.stack( [ idx_keep, zeros_keep, ], axis=1 ),
                           tf.stack( [ idx_keep, zeros_keep + 1, ], axis=1 ),
                           tf.stack( [ idx_keep, zeros_keep + 2, ], axis=1 ),
                           tf.stack( [ idx_keep, zeros_keep + 3, ], axis=1 )], axis=0)

    #print('idx_remove, idx_keep ',idx_remove.shape, idx_keep.shape)

    #print('values_remove, values_keep ',values_remove.shape, values_keep.shape)

    # now we can create a sparse matrix
    vals_remove = tf.sparse.reorder(
                  tf.sparse.SparseTensor( idx_remove, tf.concat([values_remove[:,i] for i in range(values_remove.shape[1])], axis=0), 
                  target_shape))#tf.shape(logits, out_type = tf.int64)
    vals_keep = tf.sparse.reorder(
                tf.sparse.SparseTensor( idx_keep, tf.concat([values_keep[:,i] for i in range(values_keep.shape[1])], axis=0), 
                target_shape))#tf.shape(logits, out_type = tf.int64)

    # add together the two matrices (need to convert them to dense first)
    filtered = tf.add(
        tf.sparse.to_dense(vals_remove, default_value = 0. ),
        tf.sparse.to_dense(vals_keep, default_value = 0. )
    )
...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
    ...
    pts_mask = tf.math.reduce_all(tf.math.logical_and((scene_bbox[0][None] < inputs_flat), 
                                 (inputs_flat < scene_bbox[1][None])), axis=-1)#[...,None]

    inputs_flat = renormalize_to_unit_box(inputs_flat)

    '''pcd = inputs_flat.numpy().reshape(-1,3)
    pcd_color = np.array([0,1,0])[None].repeat(pcd.shape[0],axis=0)
    print('pcd, pcd_color ',pcd.shape,pcd_color.shape)
    pcd = np.concatenate([pcd,pcd_color], axis=1)
    np.savetxt("pcd.txt", pcd, delimiter=' ', fmt='%.4f')
    print('Writing pts ',inputs_flat.shape)
    sys.exit(0)'''
    ...
    #_zeros = tf.constant(tf.zeros([inputs_flat.shape[0],4]))
    #print('type ',type(outputs_flat),outputs_flat.shape,type(embedded),embedded.shape,type(pts_mask),pts_mask.shape)
    outputs_flat = batchify(fn, netchunk)(embedded)#[pts_mask]

    #outputs_flat = filtering( tf.constant([inputs_flat.shape[0],4], dtype = tf.int64), outputs_flat, pts_mask)
    
    #print('pts_mask, outputs_flat ',pts_mask.shape, outputs_flat.shape)
    #outputs_flat = tf.where(pts_mask, outputs_flat, tf.zeros_like(outputs_flat))#
...
def render(H, W, K,
    ...
    if c2w is not None:
        ...
        print('get_rays_np_coords ',get_rays_np_coords)
        rays_o, rays_d = get_rays_np_coords(H, W, K, c2w)
    else:
...
def create_nerf(args):
    ...
    if args.ft_path is not None and args.ft_path != 'None':
        ...
        ckpts = [os.path.join(basedir, exptype, expname, f) for f in sorted(os.listdir(os.path.join(basedir, exptype, expname))) if 'model' in f]
    print('Found ckpts', ckpts, ' in ',os.path.join(basedir, exptype, expname))
...
def train():
    ...
    if args.dataset_type == 'kitti360':
        images, poses, depths_gt, test_poses, render_poses, random_poses, hwf, near, far, _ = load_kitti(
                                                        ...
                                                        initial_fov = 53.13,
                                                        reg_pose_list = None)#args.reg_pose_list

        i_train = list(range(len(images)))
        #random_poses = random_poses[0:1]
    else:
    ...
    if args.render_only:
        ...
        testsavedir = os.path.join(basedir, exptype, expname, 'renderonly_{}_{:06d}'.format(
            'test' if args.render_test else 'path', start))
        os.makedirs(testsavedir, exist_ok=True)
        print('test poses shape', render_poses.shape)

        rgbs, _ = render_path(random_poses, hwf, K, args.chunk, render_kwargs_test, 
                              gt_imgs=None, savedir=testsavedir, render_factor=args.render_factor)
    ...
    if use_batching:
        ...
        rays = [get_rays_np(H, W, K, p) for p in poses[:, :3, :4]]
    ...
    for i in tqdm(range(start, N_iters)):
        ...
        def save_weights(net, prefix, i):
            path = os.path.join(
                basedir, exptype, expname, '{}_{:06d}.npy'.format(prefix, i))
        ...
        if i % args.i_video == 0 and i > 0:
            ...
            moviebase = os.path.join(
                basedir, exptype, expname, '{}_spiral_{:06d}_'.format(expname, i))
        ...
        if i % args.i_testset == 0 and i > 0:
            testsavedir = os.path.join(
                basedir, exptype, expname, 'testset_{:06d}'.format(i))
            os.makedirs(testsavedir, exist_ok=True)
            print('test poses shape', render_poses.shape)
            render_path(random_poses, hwf, K, args.chunk, render_kwargs_test, gt_imgs=None, savedir=testsavedir)#poses[10:20]
            print('Saved test set')

        if i % args.i_print == 0 or i < 10:

            tqdm.write('Itr: ' + str(global_step.numpy()) + ' , PSNR: ' + str(psnr.numpy()) + ' , Loss: ' + str(loss.numpy()) )
            ...
            if i % args.i_img == 0:
                ...
                testimgdir = os.path.join(basedir, exptype, expname, 'tboard_val_imgs')
                if i==0:
