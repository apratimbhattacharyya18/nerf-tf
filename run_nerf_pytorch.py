...
#scene_bbox = torch.tensor([[-40.5304, -100.0, -8.15422], [ 111.638, 100.0,  30.0]]).cuda()# -8.15422, 30.0
...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
    ...
    #print('inputs_flat norm',torch.min(inputs_flat, dim=0)[0],torch.max(inputs_flat, dim=0)[0])

    #print('inputs_flat ',torch.min(inputs_flat, dim=0)[0],torch.max(inputs_flat, dim=0)[0])
    #tensor([38.1144,  0.2001, -2.5039]) tensor([ 1.8587e+02,  6.9043e+01, -1.1890e-01])
    #torch.Size([2048, 3]) tensor(1020)

    pts_mask = torch.logical_and((scene_bbox[0][None] < inputs_flat), (inputs_flat < scene_bbox[1][None])).all(dim=-1)#[...,None]

    pcd = inputs_flat[pts_mask].detach().cpu().numpy().reshape(-1,3)
    pcd_color = np.array([0,0,1])[None].repeat(pcd.shape[0],axis=0)
    ...
    print('inputs_flat norm',torch.min(inputs_flat, dim=0)[0],torch.max(inputs_flat, dim=0)[0])
    print('inputs_flat norm',torch.min(inputs_flat[pts_mask], dim=0)[0],torch.max(inputs_flat[pts_mask], dim=0)[0])
    sys.exit(0)
    
    inputs_flat = renormalize_to_unit_box(inputs_flat)

    #print('inputs_flat norm',torch.min(inputs_flat, dim=0)[0],torch.max(inputs_flat, dim=0)[0])
    #print('inputs_flat norm masked ',torch.min(inputs_flat[pts_mask], dim=0)[0],torch.max(inputs_flat[pts_mask], dim=0)[0])
    #sys.exit(0)
    ...
    '''print('pts_mask ',inputs_flat.shape, torch.sum(pts_mask))
    print('inputs_flat norm',torch.min(inputs_flat, dim=0)[0],torch.max(inputs_flat, dim=0)[0])
    #tensor([ 0.0337, -0.0522, -0.7038]) tensor([ 1.9756,  1.8364, -0.5788])
    print('inputs_flat norm masked ',torch.min(inputs_flat[pts_mask], dim=0)[0],torch.max(inputs_flat[pts_mask], dim=0)[0])
    #tensor([ 0.0337, -0.0522, -0.6416]) tensor([ 0.9999,  0.8875, -0.5788])

    np.savetxt("pcd.txt", inputs_flat[pts_mask].detach().cpu().numpy().reshape(-1,3), delimiter=' ', fmt='%.4f')
    print('Writing pts ',inputs_flat[pts_mask].shape)
    sys.exit(0)'''
...
def render(H, W, K, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    if c2w is not None:
        ...
        #coords_list = torch.tensor([[446,183],[446,190]]).cuda()
        coords_list = torch.tensor([[1408//2,376//2],[1407,0],[0,375],[0,0],[1407,375],[1408//2,0],[0,376//2]]).cuda()
        #coords_list = torch.tensor([[1408//2,0],[1408//2,375]]).cuda()
        rays_o, rays_d = get_rays_coords(H, W, K, c2w, coords_list)
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    #transmittances = torch.exp(-torch.cumsum(alpha, dim=-1).roll(1, 0))
...
def render_rays(ray_batch,
    ...
    pts = rays_o[...,None,:] + viewdirs[...,None,:] * z_vals[...,:,None] # [N_rays, N_samples, 3] ##!!
...
def train():
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            rgbs, _ = render_path(torch.Tensor(random_poses).to(device) if args.render_test else torch.Tensor(poses).to(device), 
            ...
            imageio.mimwrite(os.path.join(testsavedir, 'video.mp4'), to8b(rgbs), fps=5, quality=8)
