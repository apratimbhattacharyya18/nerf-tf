...
class Isometry():
    ...
    def tensor(self,):
        return self.cam#torch.tensor(self.cam).double()
...
class CamParams():#nn.Module
    def __init__(self, phi, t, f, H0=None, W0=None, so3_repr=None, intr_repr=None):
        ...
        self.phi = phi#nn.Parameter(phi)
        self.t = t#nn.Parameter(t)
        self.f = f#nn.Parameter(f)
    ...
    def from_file(pose_file: str, cam_pose_file: str, intrinsic_file: str, 
        ...
        print(poses.shape)
        poses_left = poses#torch.from_numpy(poses)
        poses_right = poses2#torch.from_numpy(poses2)
        '''print("========")'''
        ...
        poses_left = world_from_scene[None] @ poses_left
        poses_right = world_from_scene[None] @ poses_right
        ...
        f = np.array([552.554261, 552.554261])


        m_left = CamParams(phi_left, t_left, f, H0, W0, so3_repr, intr_repr)
        m_right = CamParams(phi_right, t_right, f, H0, W0, so3_repr, intr_repr)
    ...
    def transform_pose_list(pose_file, inv, center_idx = -1,
        ...
        poses_left = render_poses#torch.from_numpy(render_poses)
        phi_left = poses_left[:, :3, :3]
...
def interpolate_poses(poses, num_views=10):
    poses = poses[30:32]
