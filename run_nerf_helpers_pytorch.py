    ...
    def create_embedding_fn(self):
        ...
        for freq in freq_bands:
            for p_fn in self.kwargs['periodic_fns']:
                ...
                out_dim += d
                #print('p_fn, freq ',p_fn, freq)
                    
        #print('freq_bands ',freq_bands.shape, d, out_dim)
        #sys.exit(0)
        self.embed_fns = embed_fns
        self.out_dim = out_dim
        
    def embed(self, inputs):
        #encodings = torch.cat([fn(inputs) for fn in self.embed_fns], -1)
        #print('encodings ',encodings.shape)
        #return encodings
        max_freq = self.kwargs['max_freq_log2']
        N_freqs = self.kwargs['num_freqs']
        frequencies = 2.**torch.linspace(0., max_freq, steps=N_freqs)
        #print('inputs ',inputs.shape)
        #print('size -- ',len(inputs.shape) * [1] + [N_freqs],frequencies.shape)
        frequencies = torch.reshape(frequencies, len(inputs.shape) * [1] + [N_freqs])
        angles = math.pi * frequencies * torch.unsqueeze(inputs, dim=-1)
        encodings = torch.cat([torch.sin(angles), torch.cos(angles)], dim=-1)
        encodings = encodings.reshape(inputs.shape[0],-1)
        encodings = torch.cat([inputs,encodings],dim=-1)
        #print('encodings ',encodings.shape)
        return encodings
...
class NeRF(nn.Module):
    def __init__(self, D=8, W_density=256, W_viewdir=256, 
        ...
        #self.keras_model = tf.keras.models.load_model( weights_path, compile=True)
    ...
    def forward(self, x ):
        ...
        for i, l in enumerate(self.pts_linears):
            ...
            #h = F.relu(h)
            if i in self.skips:
                h = torch.cat([input_pts, h], -1)#h, input_pts
    ...
    def load_weights_from_pnf(self, weights_path):
        ...
        '''inputs = {
              'ray_origins': tf.zeros([32,3], tf.float32),
              'ray_directions': tf.zeros([32,3], tf.float32),
        }
        print(model(inputs))
        ...
        for i in range(self.D):
            #print('model._coarse_scene_mlp.hidden_layers[i].weights[0] ',model._coarse_scene_mlp.hidden_layers[i].weights[0].shape)
...
def get_rays_coords(H, W, K, c2w, coords_list):
    i, j = torch.meshgrid(torch.linspace(0, W-1, W), torch.linspace(0, H-1, H))  # pytorch's meshgrid has indexing='ij'
    i = i.t()
    j = j.t()
    #print('i, j ',torch.min(i),torch.min(j))
    #print('i, j ',torch.max(i),torch.max(j))
    #sys.exit(0)
    i = coords_list[:,0]
    j = coords_list[:,1]
    dirs = torch.stack([(i-K[0][2])/K[0][0], (j-K[1][2])/K[1][1], torch.ones_like(i)], -1)
