...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
    ...
    #outputs_flat = tf.zeros([inputs_flat.shape[0],4])
...
def render_rays(ray_batch,
    ...
    def raw2outputs(raw, z_vals, rays_d):
        ...
        disp_map = depth_map#1./tf.maximum(1e-10, depth_map / tf.reduce_sum(weights, axis=-1))
...
def render(H, W, K,
    ...
    if c2w is not None:
        ...
        #print('rays ',type(rays), rays.shape)
        rays_o = rays[0]
        rays_d = rays[1]
    ...
    return tuple(ret_list + [ret_dict])
...
def train():
    ...
    if args.render_only:
        ...
        rgbs, _ = render_path(poses, hwf, K, args.chunk, render_kwargs_test, 
    ...
    def train_step(inputs):
        ...
        with tf.GradientTape() as tape:
            ...
            rgb, disp, acc, extras = outs

            # Compute MSE loss between predicted and true RGB.
            img_loss = img2mse_scaled(rgb, target_s, args.N_rand)
            trans = extras['raw'][..., -1]
            loss = img_loss
    ...
    for i in tqdm(range(start, N_iters)):
        ...
        #per_replica_losses = strategy.run(train_step, args=(batch_rays, target_s))
        #loss = strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_losses, axis=None)
        #print('batch_rays, target_s ',batch_rays.shape, target_s.shape)
        loss = distributed_train_step((batch_rays, target_s))
        psnr = mse2psnr(loss)
        ...
        if i % args.i_testset == 0 and i > 0:
            ...
            os.makedirs(testsavedir, exist_ok=True)
            print('test poses shape', render_poses.shape)
            render_path(poses[10:20], hwf, K, args.chunk, render_kwargs_test, gt_imgs=None, savedir=testsavedir)
